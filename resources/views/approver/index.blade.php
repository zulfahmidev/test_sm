...
@section('content')
  <h1 class="text-3xl">Penyetujui</h1>
  <div class="bg-white rounded px-6 lg:px-12 py-6 lg:py-10 shadow my-5">
    <div class="flex justify-between gap-2">
      <a href="{{ route('approver.create') }}" class="bg-blue-500 hover:bg-blue-600 cursor-pointer flex text-white rounded">
      ...
      <table class="table mt-3 text-center">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="capitalize">{{ $approver->fullname }}</td>
            <td>{{ $approver->username }}</td>
            <td>
              <a href="{{ route('approver.edit', ['approver' => $approver->id]) }}" class="block justify-center py-2 w-full bg-green-400 hover:bg-green-500 cursor-pointer text-white rounded">
                <i class="fa fa-edit"></i>
              </a>
            </td>
            <td>
              <form action="{{ route('approver.destroy', ['approver' => $approver->id]) }}" method="post">
                @csrf @method('DELETE')
