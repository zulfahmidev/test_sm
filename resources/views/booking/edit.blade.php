...
@section('content')
  <h1 class="text-3xl">Sunting Data Pemesanan</h1>
  <div class="bg-white rounded px-6 lg:px-12 w-full lg:max-w-lg py-6 lg:py-10 shadow my-5">
    ...
    <form action="{{ route('booking.update', ['booking' => $booking->id]) }}" method="post">
      ...
      <div class="mb-2">
        ...
        <input type="text" value="{{ $booking->customer_name }}" placeholder="ketik disini..." class="input" id="customer_name" name="customer_name">
        @error('customer_name')
      ...
      <div class="mb-2">
        ...
        <input type="datetime-local" value="{{ $booking->usage_at }}" placeholder="ketik disini..." class="input" id="usage_at" name="usage_at">
        @error('usage_at')
        <div class="text-xs text-red-500 mt-1">{{ $message }}</div>
        @enderror
      </div>
      <div class="mb-2">
        ...
        <input type="number" value="{{ $booking->estimated_duration }}" placeholder="ketik disini..." value="1" min="1" class="input" id="estimated_duration" name="estimated_duration">
      ...
      <div class="mb-2">
        ...
        <textarea name="note" placeholder="ketik disini..." id="note" class="input" cols="30" rows="5">{{ $booking->note }}</textarea>
        @error('note')
        <div class="text-xs text-red-500 mt-1">{{ $message }}</div>
        @enderror
      </div>
      ...
      <div class="mb-2">
        ...
        <input type="hidden" name="vehicle_id" id="vehicle_id" value="{{ $booking->vehicle_id }}">
        <div class="px-3 py-2 bg-gray-50 border border-gray-400 rounded flex justify-between items-center cursor-pointer" id="vehicle_select" onclick="vehicleOptions()">
          <span>{{ $booking->getVehicle()->plat . ' - ' . translateStatus($booking->getVehicle()->transport_type) }}</span>
      ...
      <div class="mb-2">
        ...
        <input type="hidden" name="driver_id" id="driver_id" value="{{ $booking->driver_id }}">
        <div class="px-3 py-2 bg-gray-50 border border-gray-400 rounded flex justify-between items-center cursor-pointer" id="driver_select" onclick="driverOptions()">
          <span>{{ $booking->getDriver()->fullname . ' - ' . $booking->getDriver()->phone }}</span>
          <i class="fa fa-angle-down"></i>
        </div>
      ...
      <div class="mb-2">
        ...
        <div class="px-3 py-2 bg-gray-50 border border-gray-400 rounded flex justify-between items-center cursor-pointer" id="approver_select" onclick="approverOptions({{$booking->id}})">
        ...
        <div id="approvers" class="flex flex-wrap gap-2 mt-2">
          @foreach ($booking->getApprovers() as $approver)
          <input type="hidden" name="user_id[]" value="{{ $approver->id }}">
          <div class="px-3 py-2 rounded border border-gray-400 w-full">
